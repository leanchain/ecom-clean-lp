...
- **Give the scan its own front door.** `/tools/ai-visibility-scan` is a real self-serve entry point and it is linked from exactly one place: the bottom-left of the footer, sitemap priority 0.3. A cold visitor currently has one option — book a call. Put the scan above the fold as the secondary CTA.
