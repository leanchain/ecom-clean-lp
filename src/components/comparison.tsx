      ...
      <div className="container">
        <div className="flex flex-col gap-6 text-center md:gap-12">
          <h2 className="mb-2 text-3xl font-bold md:text-5xl">
            Fisca PDP AI vs Traditional Product Pages
          </h2>
          <p className="text-muted-foreground mx-auto max-w-2xl text-lg">
            Fisca PDP AI creates deep narrative product page content engineered
        ...
        <div className="mt-20 grid gap-12 lg:grid-cols-2 lg:gap-0">
          ...
          <div className="rounded-3xl border border-y p-6 lg:rounded-l-none lg:border-l-0 lg:p-12">
            <h3 className="text-2xl font-medium">Fisca PDP AI</h3>
            <ul className="mt-9 space-y-3">
              {FiscaApproach.map((feature, idx) => (
                <React.Fragment key={idx}>
                  <li className="flex items-center gap-2 text-sm">
                    <span className="text-lg">{feature.emoji}</span>
                    {feature.text}
                  </li>
                  {idx !== FiscaApproach.length - 1 && <Separator />}
                </React.Fragment>
              ))}
            </ul>
          </div>
