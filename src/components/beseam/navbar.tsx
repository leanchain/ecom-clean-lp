              ...
              <div className="pointer-events-none absolute left-1/2 top-[calc(100%+0.55rem)] w-[49rem] max-w-[calc(100vw-2.5rem)] -translate-x-1/2 translate-y-2 opacity-0 transition duration-150 group-hover:pointer-events-auto group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:translate-y-0 group-focus-within:opacity-100">
                <div className="border border-black/22 bg-[#f4f1e9] shadow-[0_24px_60px_rgba(17,19,24,0.14)]">
                  <div className="grid grid-cols-3">
                    {PRODUCT_SYSTEMS.map((system, index) => (
                      <Link
                        key={system.name}
                        href={system.href}
                        className="group/item border-r border-black/18 p-6 last:border-r-0 hover:bg-[#ebe8df] focus-visible:bg-[#ebe8df]"
                      >
                        <div className="flex items-baseline justify-between gap-4">
                          <span className="font-serif text-[24px] tracking-[-0.025em] text-[#111318]">
                            {system.name}
                          </span>
                        ...
                        <p className="mt-3 min-h-10 text-[13px] leading-relaxed text-black/58">
                          {system.statement}
                        </p>
                        <div className="mt-6 border-t border-black/18 pt-4">
                          {system.products.map((product) => (
                            <span
                              key={product}
                              className="block py-1 font-mono text-[10px] uppercase tracking-[0.08em] text-black/48"
                            >
                              {product}
                            </span>
                          ))}
                        </div>
                        <span className="mt-5 inline-flex items-center gap-2 text-[12px] font-semibold text-[#3154ff]">
                          View products{" "}
                  ...
                  <div className="flex items-center justify-between border-t border-black/18 px-6 py-3 font-mono text-[10px] uppercase tracking-[0.08em] text-black/45">
                    ...
                    <Link href="/#packages" className="text-[#3154ff]">
                      Compare products →
                    </Link>
                  </div>
                </div>
        ...
        >
          <div className="mx-auto max-w-[92rem] px-5 pb-7 sm:px-8">
            <div className="border-b border-black/18 py-5">
              <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.12em] text-[#3154ff]">
                Products by job
              </p>
              {PRODUCT_SYSTEMS.map((system) => (
                <Link
                  ...
                  className="grid grid-cols-[5.5rem_1fr] gap-4 border-b border-black/14 py-4 last:border-b-0"
                >
                  <span className="font-serif text-[20px] text-[#111318]">
                    {system.name}
                  </span>
            ...
            <nav
              aria-label="Mobile"
              className="grid grid-cols-2 border-b border-black/18 py-3"
            >
              {NAV_LINKS.map((link) => (
                <Link
                  key={link.label}
                  href={link.href}
                  onClick={() => setMobileOpen(false)}
                  className="py-3 text-[14px] font-semibold text-black/68"
                >
                  {link.label}
                </Link>
              ))}
