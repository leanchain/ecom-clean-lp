...
const SEVERITY: Record<Severity, { label: string; className: string }> = {
  ...
  medium: { label: "Medium", className: "text-black/62" },
};
    ...
    <article className="border-b border-black/18">
      <h3>
        ...
        >
          ...
          <span
            className={`hidden font-mono text-[12px] font-semibold uppercase tracking-[0.1em] sm:block ${severity.className}`}
          >
            {severity.label}
          </span>
          <span className="text-[16px] font-semibold leading-snug text-black/82">
            <span
              className={`mb-1 block font-mono text-[12px] font-semibold uppercase tracking-[0.1em] sm:hidden ${severity.className}`}
            >
              {severity.label}
            </span>
            {finding.title}
          ...
          <ChevronDown
            aria-hidden="true"
            className={`mt-1 h-4 w-4 shrink-0 text-black/62 transition-transform duration-200 ${open ? "rotate-180" : ""}`}
          />
        </button>
      </h3>
      {open ? (
        <div className="pb-7 sm:pl-[8.5rem]">
          ...
          <p className="mt-6 font-mono text-[12px] font-semibold uppercase tracking-[0.1em] text-[#b8441d]">
            Proposed fix
          </p>
