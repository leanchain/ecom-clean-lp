...
This folder contains archived components, routes, and assets from the original Sonic speaker template that are no longer needed for the live Fieson AI PDP product.
...
- `sections/why-fieson.tsx` ← Has correct Fieson copy
...
### Data (`data/`)
- `products.json` - 4 "Fieson Light - Premium Smart Soundbar" products with speaker specs
...
All content in this archive is related to the original **Sonic speaker template** and is not relevant to the **Fieson AI PDP product**. The main repository now contains only Fieson-specific content for AI-powered product detail page generation.
...
These files are kept for historical reference and potential template reuse. They are not part of the live Fieson product and should not be imported into active code.
